...
knowledge = "../datasets/knowledges.hdf5"
knowledge_file = "../datasets/knowledge.json"
...
class ReverieTextPathData(object):
    def __init__(
        ...
        max_txt_len=100, in_memory=True, act_visited_node=False, knowledge_file = knowledge, knowledge_files = knowledge_file
    ):
        ...
        self.viewpoint_knowledge = json.load(open(knowledge_files, 'r'))
        # self.knowledge = json.load(open(knowledge_file))
    ...
    def knowledge_db(self, key):
        if self.in_memory and key in self._feature_store:
            ft = self._feature_store[key]
        else:
            with h5py.File(self.knowledge_file, 'r') as f:
    ...
    def get_input(
        ...
        traj_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids, \
            last_vp_angles, last_vp_objids, knowledges = self.get_traj_pano_fts(scan, gt_path)
    ...
    def get_traj_pano_fts(self, scan, path):
        ...
        for vp in path:
            ...
            for i in range(36):
                view_feature = []
                knowledge_ids = self.viewpoint_knowledge[key + '_' + str(i)][:9]

                for k in knowledge_ids:
                    augment_feature = self.knowledge_db(str(k))
                    view_feature.append(augment_feature.reshape(1, 512))
                knowledge_feature.append(view_feature.reshape(1, 9, 512))
            knowledge_fts.append(knowledge_feature.reshape(36, 9, 512))

        knowledge_fts = torch.tensor(knowledge_fts, axis=0)

        return traj_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids, \
               last_vp_angles, last_vp_objids, knowledge_fts
...
class R2RTextPathData(ReverieTextPathData):
    def __init__(
        ...
        image_feat_size=2048, image_prob_size=1000, angle_feat_size=4,
        max_txt_len=100, in_memory=True, act_visited_node=False, knowledge_file = knowledge
    ):
        super().__init__(
            ...
            act_visited_node=act_visited_node, knowledge_file = knowledge
        )
    ...
    def get_input(
        ...
        traj_view_img_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids, \
            last_vp_angles, knowledges = self.get_traj_pano_fts(scan, gt_path)
