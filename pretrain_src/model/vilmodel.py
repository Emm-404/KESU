...
class GlocalTextPathCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.knowledges_vp_encoder = CrossmodalEncoder(config)
    ...
    def forward(
        ...
        knowledges_lens, return_gmap_embeds=True
    ):
        ...
        knowledges_embeds = self.knowledge_proj(knowledges.view(batch_size, -1, 512))
        ...
        fuse_embeds = torch.cat([vp_embeds, gmap_embeds], dim=1)
        ...
        vp_txt_embeds = self.local_encoder.encoder(txt_knowledges_embeds, txt_masks, vp_knowledges_embeds, fuse_masks)
        gmap_embeds = vp_txt_embeds[:, vp_embeds.shape[1]:]
        vp_embeds = vp_txt_embeds[:, :vp_embeds.shape[1]]
    ...
    def forward_mlm(
        ...
        txt_knowledges_embeds = txt_embeds
        for layer_module in self.knowledges_text_encoder.x_layers:
            txt_knowledges_embeds = layer_module.forward_lang2visn(
                txt_embeds, extended_txt_masks,
                knowledges_embeds, None,
        ...
        fuse_embeds = torch.cat([vp_input_embeds, gmap_input_embeds], dim=1)
        fuse_masks = torch.cat([vp_masks, gmap_masks], dim=1)
        extend_fuse_masks = extend_neg_masks(fuse_masks)

        vp_knowledges_embeds = self.knowledges_vp_encoder(knowledges_embeds, None, fuse_embeds, fuse_masks)

        for layer_module in self.local_encoder.encoder.x_layers:
            txt_embeds = layer_module.forward_lang2visn(
                txt_knowledges_embeds, extended_txt_masks,
                vp_knowledges_embeds, extend_fuse_masks,
            )
