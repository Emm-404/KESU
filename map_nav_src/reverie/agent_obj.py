...
# knowledge_file = "../datasets/R2R/captions-tokenizer.json"
# knowledge_ = json.load(open(knowledge_file))
...
class GMapObjectNavAgent(Seq2SeqAgent):
    ...
    def _language_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            knowledges.append(ob['knowledge_feature'])

        knowledge_fts = torch.tensor(knowledges, axis=0).cuda()
        ...
        return {
            ...
            'knowledges': knowledge_fts, 'knowledges_lens': knowledges_lens
        }
    ...
    def _nav_gmap_variable(self, obs, gmaps):
        ...
        knowledge_fts = torch.tensor(knowledges).cuda()
        knowledges_lens = torch.LongTensor([len(x) for x in knowledge_fts]).cuda()
