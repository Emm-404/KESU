...
class EnvBatch(object):
    ...
    def __init__(self, connectivity_dir, scan_data_dir=None, feat_db=None, batch_size=100):
        ...
        """
        self.knowledge_db = KnowledgeDB("../datasets/knowledges.hdf5")
        self.knowledge = json.load(open('../datasets/knowledge.json', 'r'))
...
class R2RNavBatch(object):
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            for i in range(36):
                view_feature = []
                knowledge_ids = self.env.knowledge[key + '_' + str(i)][:9]

                for k in knowledge_ids:
                    augment_feature = self.env.knowledge_db.get_feature(str(k))
                    view_feature.append(augment_feature)
                knowledge_feature.append(view_feature)
